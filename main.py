...
gravity = 0.1  # How fast bird falls (reduced from 0.2)
flap_strength = -0.5  # How much a flap lifts the bird (reduced from -0.8)
player_velocity = 0  # Current velocity
score = 0
game_speed = 300  # Controls overall game speed (higher = slower)
...
def reset_game():
    global player_y, player_velocity, score, walls
    player_y = 2
    player_velocity = 0
    score = 0
    walls = [[4, random.randint(1, 3)]]  # Reset with one wall
...
while True:
    ...
    # Make sure player_y is an integer for display
    ...
    if button_a.is_pressed():
        player_velocity = flap_strength  # Flap!
        sleep(100)  # Small delay to prevent multiple flaps
    ...
    # Only move walls every game_speed milliseconds
    current_time = running_time()
    if current_time % game_speed < 20:  # This makes walls move slower
        ...
        for i in range(len(walls)):
            ...
            if walls[i][0] == 1:
                ...
                if abs(int(player_y) - walls[i][1]) <= 1:
                    ...
                    sleep(300)  # Longer pause when scoring
                else:
                    # Bird hit the wall
                    show_game_over()
                    reset_game()
